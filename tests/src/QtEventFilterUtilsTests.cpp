...
void QtEventFilterUtilsTest::test_filter() const {
  ...
  oclero::EventFilter<QEvent::Type::ContextMenu>::install(dummyObject, [&called](QContextMenuEvent* e) {
    called = true;
    return false;
  });

  // Send this dummy event synchronously.
  QContextMenuEvent dummyEvent(QContextMenuEvent::Reason::Other, QPoint{ 0, 0 });
